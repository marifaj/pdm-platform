...
def load_model():
    ...
    if isinstance(artifact, dict) and "model" in artifact:
        ...
        if trained_feature_columns is not None and trained_feature_columns != FEATURE_COLUMNS:
            raise ValueError(
        ...
        log(
            f"[BOOT] window model loaded from {MODEL_PATH} "
            f"(trained_window_size={trained_window_size}, "
            f"trained_window_step={trained_window_step})"
        )
        return m
...
def combine_decision(is_anomaly_ml: int, is_anomaly_ewma: int):
    if DETECTION_MODE == "ewma_only":
        ...
        is_final = 1 if ((is_anomaly_ml or is_anomaly_ewma) if HYBRID_USE_OR else (is_anomaly_ml and is_anomaly_ewma)) else 0
...
def build_window_result(window_rows, window_features, if_score, if_pred, is_anomaly_ml):
    ...
    is_anomaly_final, anomaly_reason = combine_decision(is_anomaly_ml, is_anomaly_ewma)
...
def process_window_if_ready(client, device_id: str):
    ...
    while len(buf) >= WINDOW_SIZE:
        ...
        result = build_window_result(window_rows, window_features, if_score, if_pred, is_anomaly_ml)
...
def on_message(client, userdata, msg):
    try:
        ...
        temp_ewma, temp_residual, temp_zscore, is_anomaly_ewma = temp_monitor.update(row["temperature_c"])
